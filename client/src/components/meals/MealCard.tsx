    ...
    <div className={`border rounded-lg overflow-hidden ${
      ...
      <div className="p-4">
        <div className="flex space-x-4 mb-3">
          ...
          {hasBeforePhoto ? (
            ...
            <div
              className={`flex-1 border-2 border-dashed rounded-md overflow-hidden h-32 relative flex flex-col items-center justify-center ${
                isActive ? "border-primary-300 bg-primary-50" : "border-gray-300 bg-gray-50 opacity-50"
              }`}
            >
              <div className="flex items-center justify-center flex-col">
                <div className={`h-12 w-12 rounded-full flex items-center justify-center mb-2 ${
                  ...
                  <Camera className={`text-xl ${isActive ? "text-primary-600" : "text-gray-500"}`} />
                </div>
                <p className={`text-sm font-medium ${isActive ? "text-primary-700" : "text-gray-500"}`}>
                  Take 'Before' Photo
                </p>
              </div>
              {isActive && (
          ...
          {hasAfterPhoto ? (
            ...
            <div
              className={`flex-1 border-2 border-dashed rounded-md overflow-hidden h-32 relative flex flex-col items-center justify-center ${
                isActive && hasBeforePhoto ? "border-primary-300 bg-primary-50" : "border-gray-300 bg-gray-50 opacity-50"
              }`}
            >
              <div className="flex items-center justify-center flex-col">
                <div className={`h-12 w-12 rounded-full flex items-center justify-center mb-2 ${
        ...
        <div className="flex items-center justify-between border-t pt-3 border-gray-200">
          ...
          <Button
            ...
            className={`px-4 py-1 rounded-full text-sm h-auto ${
              isCompleted ? "bg-gray-200 text-gray-500" : "bg-primary text-white hover:bg-primary/90"
            }`}
            onClick={() => {
              if (!hasBeforePhoto) {
      ...
      {showBeforeCamera && (
        <CameraCapture
          ...
          onSuccess={() => setShowBeforeCamera(false)}
        />
      )}
      ...
      {showAfterCamera && (
        <CameraCapture
          ...
          onSuccess={() => setShowAfterCamera(false)}
        />
      )}
    </div>
  );
